...
Home leads to the real terminal for the selected agent. Moving between home and terminal must preserve the person's place.

A blocked-agent notification opens that agent's terminal, not generic Home.

## Terminal

The terminal favors fidelity over decoration. Preserve selection, keyboard input, control keys, scrolling, resize behavior, reconnect state, and text paste. Use only capabilities Herdr already exposes.
...
- Preserve reading and scroll position between Home and Terminal.
