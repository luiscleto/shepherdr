...
Human-approved default bar order: **Message**, **Send keys**, **Release**, **Esc**, **Enter**, **Up**, **Down**, **Backspace**, **Ctrl+C**. **Send keys** is text-only, with no icon, immediately after Message. Preserve the existing lifecycle-dependent **Control**, confirmed **Take over**, and **Release** behavior. **Message** (or the existing **Attach files** action in full Terminal), **Enter**, **Send keys**, and actions that cannot sensibly be represented by a key picker remain fixed and non-removable, with their existing availability rules. Fixed Enter follows the first optional shortcut, or the control action when none are saved.
...
Keys act on the open terminal's current state, independently of any unsent Reader message or pending files. Sending a key never submits or clears that draft. Close the Send keys sheet only after Herdr confirms acceptance, with no routine success message. Failed or unknown results keep it open with their notice and deliberate recovery. Selecting, pinning, or editing does not close it as a successful send; a saved bar shortcut does not open the sheet on success. Keep the existing controller; while observing, use **Control** or confirmed **Take over** first, then deliberately send the key. The key action itself never takes over. Let the terminal determine the effect of a combination, including ordinary aliases.
...
A person can pin a shortcut without sending it, then edit, remove, or reorder optional saved shortcuts in this browser, or **Restore defaults**. Following human phone feedback, start with **Esc**, **Up**, **Down**, **Backspace**, and **Ctrl+C**, in that order. Preserve existing saved choices; Restore defaults applies this list. **Tab** and other existing keys remain reachable through the picker and can be pinned. There is no arbitrary shortcut-count cap. Selecting a key, changing modifiers, pinning, and editing never send input. Modifiers belong to the selected combination, not to ordinary typing or other actions. Keep Message, files, native keyboard input, and the rest of Terminal intact. Do not add macros, profiles, global sticky modifiers, or provider-specific behavior. The [approved Send keys brief](send-keys-plan.md) records the Herdr encoding route, server safeguards, capability evidence and human-accepted limitations.
