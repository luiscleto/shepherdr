...
- One deny-by-default route inventory requires exact Host on every request, including health and fixed assets. Mutations and WebSocket upgrades reject absent, `null`, duplicate, malformed, or noncanonical Origin. Legitimate Origin-less authenticated reads still require exact Host and a valid session. Mutations are JSON-only, GET never changes state, and the approved cookie constraints hold: host-only `Secure`, `HttpOnly`, and `SameSite=Strict`, with no separate CSRF token.
...
9. Require exact Host on health, fixed assets, authenticated reads, mutations, and WebSockets. For mutations and WebSockets, reject absent, `null`, duplicate, malformed, and noncanonical Origin as well as localhost, IP, aliases, alternate ports, forwarded-header forgeries, non-JSON mutations, state-changing GETs, unknown APIs, oversized bodies, and cross-origin Home, Terminal, and optional lab WebSockets. Confirm legitimate Origin-less authenticated reads work only with exact Host and a valid session, and every current route is classified.
