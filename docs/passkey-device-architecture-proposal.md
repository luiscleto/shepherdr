...
Successful registration and sign-in also record their asserting credential and time as the initial fresh verification. Every invalidation—sign-out, authentication or reauthentication rotation, capacity eviction, finite expiry, credential revoke, and reset—first makes the old session unusable, then closes its bound Home and Terminal WebSockets, stops their per-socket control/subprocess children, and cancels any in-flight ordinary Terminal read or file-send request plus its per-request Herdr child before the operation reports success. A canceled file send performs the appropriate best-effort rollback when forwarding definitely did not happen; a possibly forwarded result remains unknown and retains staged files for workspace cleanup. A process crash closes children by process death. A timer enforces finite expiry even without a new HTTP request. Each Terminal input/takeover and each Home, workspace, or file-send authority use rechecks the bound session immediately before forwarding; authority accepted by Herdr before invalidation cannot be recalled.
...
| `POST /api/terminal/files` base64-JSON mutation | Valid bound session, exact `Origin`, exact JSON content type, configured finite or `none` body handling, exact current workspace/pane/terminal/recognized-agent checks, and authority rechecks before each part of the shared Terminal batch. Revocation cancels staging or forwarding, waits for rollback or unknown-result handling, and releases the session lease before completing. |
...
The application keeps `frame-ancestors 'none'`, `form-action 'none'`, no-referrer policy, no CORS, and text-only handling for untrusted displayed content. The narrow `img-src blob:` exception permits only browser-local pending-image thumbnails; object URLs are revoked when discarded and never grant application authority. Herdr output, agents, Terminal content, repository files, uploaded bytes, paths, IDs, attachments, pasted content, device labels, and all browser strings are untrusted. None may choose an access record, authorizer, redirect, route, Terminal target, subscription owner, invitation, or mutation. Authentication destinations are fixed same-origin paths.
