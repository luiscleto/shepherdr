...
| `GET /api/terminal` and `GET /api/terminal/read` WebSockets | Valid session and exact `Origin`; recheck before input, takeover, release, or control forwarding; stop child/control ownership on invalidation. |
| `GET /api/terminal-lab` and `GET /api/terminal-lab/read` when explicitly enabled | Same Terminal rules; enabling the lab never bypasses access. |
...
| Sign-in and trust begin/finish | No application session for sign-in/trust; exact Host/Origin, JSON, bounded body, ceremony cookie, one-use challenge, and no application data. |
