...
Shepherdr supports Herdr 0.8.0 through 0.9.0 and uses their existing snapshot, terminal read, observe, control, acknowledgement, release, and takeover capabilities. The Herdr CLI and server must have matching protocols. Real 0.9.0 workflow validation is pending. Herdr remains authoritative for the terminal process, output, and control.
