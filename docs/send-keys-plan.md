...
Use the approved paper sheet: Ctrl, Alt/Option, Shift, plus confirmed Super/Command and Hyper modifiers; Common/F keys/Character; one base plus modifiers; readable preview and explicit Send; pin without sending; browser-local edit/remove/reorder and Restore defaults. **Send keys is text-only, immediately after Message.** Following human phone feedback, default bar order is Message, Send keys, Release, Esc, Enter, Up, Down, Backspace, Ctrl+C. Optional shortcuts are Esc, Up, Down, Backspace, Ctrl+C; fixed Enter follows the first optional shortcut, or the control action when none are saved. There is no arbitrary saved-count cap. Preserve saved choices; Restore defaults applies the new list. Tab remains in the picker and pinnable. Alt/Option means terminal Alt. Keep the same three categories and wrap modifier controls as needed; no additional category is necessary. Keep macros, profiles, global sticky modifiers and provider logic out.
...
| Backspace | Explicit Common key + optional shortcut; default saved; independent of phone keyboard |
