...
- `worktree.create` with a top-level workspace ID, optional branch, and no focus;
...
- Other open workspace rows offer **Close workspace** when Herdr can close them.
...
3. Confirm that a working or blocked agent produces the interruption warning with exact counts.
4. Close one linked workspace; its processes end while its checkout and branch remain.
5. Confirm and close one disposable top-level group; the dialog names the whole group and its affected agents.
6. Dirty a disposable linked checkout and attempt deletion. Herdr refuses it and nothing disappears.
7. Clean that checkout outside Shepherdr, retry, and verify that the checkout and workspace disappear while the branch remains.
8. Submit a stale confirmation and concurrent actions from two phone tabs; neither may target the wrong workspace or invent success.
9. Recheck grouping, filtering, attention, expansion, scroll, focus, return place, and the existing terminal destination.
