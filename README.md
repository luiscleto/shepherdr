...
Download the archive for this machine from the [latest release](https://github.com/luiscleto/shepherdr/releases/latest), plus its matching checksum file. The examples below use v0.4.0.

| System | Archive |
| --- | --- |
| Linux x86-64 | `shepherdr_0.4.0_linux_amd64.tar.gz` |
| Linux ARM64 | `shepherdr_0.4.0_linux_arm64.tar.gz` |
| macOS Intel | `shepherdr_0.4.0_darwin_amd64.tar.gz` |
| macOS Apple silicon | `shepherdr_0.4.0_darwin_arm64.tar.gz` |

macOS builds are unsigned. Expect a system warning on first run.

```sh
archive=shepherdr_0.4.0_linux_amd64.tar.gz
grep "  ${archive}$" shepherdr_0.4.0_checksums.txt | sha256sum -c -
tar -xzf "$archive"
...
# or pin a version: go install github.com/luiscleto/shepherdr@v0.4.0
```
...
To upgrade to v0.4.0, update Herdr to at least 0.9.0 first. Stop Shepherdr, keep its state and configuration, replace the binary with the verified download, check `shepherdr --version`, then restart it.
